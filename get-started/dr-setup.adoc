...
[role="lead"]
NetApp Disaster Recovery를 사용하려면 NetApp Console과 지원되는 재해 복구 환경 모두에서 몇 가지 설정 단계를 수행해야 합니다.
...
* *AWS Site-Site VPN*: 이 가상 사설망 연결은 공용 인터넷 서비스 공급자와 함께 안전한 액세스 연결을 제공하는 데 사용할 수 있습니다. VPN은 AWS 클라우드로 오가는 모든 데이터를 암호화하고 복호화합니다. VPN은 소프트웨어 기반 또는 하드웨어 기반일 수 있습니다. 엔터프라이즈 애플리케이션의 경우, 공용 인터넷 서비스 공급자(ISP)는 DR 복제를 위해 적절한 대역폭과 지연 시간이 제공되도록 서비스 품질 보장을 제공해야 합니다.
