...
[role="lead"]
NetApp Disaster Recovery를 사용하려면 NetApp Console과 지원되는 재해 복구 환경에서 몇 가지 설정 단계를 완료해야 합니다.
...
* 온프레미스 VMware plus ONTAP 데이터 센터를 VMware Cloud on AWS 또는 Elastic VMware Service(EVS) 및 Amazon FSx for NetApp ONTAP 기반 AWS DR 인프라에 복제하는 하이브리드 클라우드 DR입니다.
* 온프레미스 VMware와 ONTAP vCenter를 다른 온프레미스 VMware와 ONTAP vCenter로 복제하는 프라이빗 클라우드 DR입니다.
* VMware Cloud on AWS 또는 EVS 기반의 AWS DR 인프라를 FSx for NetApp ONTAP를 사용하여 다른 AWS 기반 DR 인프라로 복제하는 Cloud DR입니다.
* 온프레미스 VMware Plus ONTAP 데이터센터를 Google Cloud VMware Engine 및 Google Cloud NetApp Volumes 기반의 Google DR 인프라로 복제하는 하이브리드 클라우드 재해 복구(DR) 솔루션입니다.
...
운영 사이트와 DR 사이트는 AWS 호환 보안 연결로 연결됩니다. 일반적인 연결 유형으로는 보안 VPN(프라이빗 또는 AWS 제공), AWS Direct Connect 또는 기타 승인된 상호 연결 방식이 있습니다.
...
*모범 사례*: AWS 재해 복구 사이트 구성의 경우, NetApp은 VMware Cloud와 AWS FSx for ONTAP 인스턴스 모두에 대해 단일 AZ 배포를 권장합니다. AWS를 재해 복구에 사용하는 경우 여러 가용 영역을 도입할 이점이 없습니다. 다중 AZ 배포는 비용과 복잡성을 증가시킬 수 있습니다.
...
* *AWS 사이트 간 VPN*: 이 가상 사설망(VPN) 연결은 공용 인터넷 서비스 제공업체(ISP)를 통해 안전한 연결을 제공합니다.  VPN은 AWS 클라우드로 오가는 모든 데이터를 암호화하고 복호화합니다.  VPN은 소프트웨어 기반 또는 하드웨어 기반일 수 있습니다.  기업 애플리케이션의 경우, 공용 인터넷 서비스 제공업체(ISP)는 DR 복제를 위해 적절한 대역폭과 지연 시간을 보장하는 서비스 품질(QoS) 보장을 제공해야 합니다.
...
*모범 사례*: VMware Cloud와 단일 FSx for ONTAP VPC를 사용하는 재해 복구(DR) 솔루션의 경우 NetApp은 VPC 피어링을 권장합니다. 여러 개의 FSx for ONTAP VPC가 배포된 경우, 여러 VPC 피어 연결에 따른 관리 오버헤드를 줄이기 위해 AWS Transit Gateway를 사용하는 것을 권장합니다.
