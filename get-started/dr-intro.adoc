...
[role="lead"]
클라우드 재해 복구는 사이트 장애 및 데이터 손상 사고로부터 워크로드를 보호하는 탄력적이고 비용 효율적인 방법입니다. NetApp Disaster Recovery를 사용하면 ONTAP 스토리지를 실행하는 온프레미스 VMware VM 또는 데이터스토어 워크로드를 NetApp 클라우드 스토리지를 사용하는 퍼블릭 클라우드의 VMware 소프트웨어 정의 데이터 센터로 복제하거나, ONTAP 스토리지를 사용하는 다른 온프레미스 VMware 환경을 재해 복구 사이트로 복제할 수 있습니다. 또한 Disaster Recovery를 사용하여 VM 워크로드를 한 사이트에서 다른 사이트로 마이그레이션할 수도 있습니다.

NetApp Disaster Recovery는 재해 복구 워크플로우를 자동화하는 클라우드 기반 재해 복구 서비스입니다. NetApp Disaster Recovery를 사용하면 온프레미스 Kubernetes 클러스터, NFS 기반 워크로드, 그리고 NetApp 스토리지를 사용하는 iSCSI 및 FC용 VMware vSphere 가상 머신 파일 시스템(VMFS) 데이터스토어를 다음 중 하나로 보호할 수 있습니다:
...
Disaster Recovery는 재해 복구 워크로드를 제공하기 위해 다음과 같은 구성 요소를 사용합니다.
