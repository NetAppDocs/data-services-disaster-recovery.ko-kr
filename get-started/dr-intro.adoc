...
summary: 클라우드 재해 복구는 사이트 장애 및 데이터 손상 사고로부터 워크로드를 보호하는 탄력적이고 비용 효율적인 방법입니다. NetApp Disaster Recovery를 사용하면 ONTAP 스토리지를 실행하는 온프레미스 VMware 워크로드를 NetApp 클라우드 스토리지를 사용하는 퍼블릭 클라우드의 VMware 소프트웨어 정의 데이터 센터로 복제하거나, ONTAP 스토리지를 사용하는 다른 온프레미스 VMware 환경을 재해 복구 사이트로 복제할 수 있습니다. 
---
...
[role="lead"]
클라우드 재해 복구는 사이트 장애 및 데이터 손상 사고로부터 워크로드를 보호하는 탄력적이고 비용 효율적인 방법입니다. NetApp Disaster Recovery는 재해 복구 워크플로우를 자동화하는 클라우드 기반 재해 복구 서비스입니다. NetApp Disaster Recovery를 사용하면 온프레미스 Kubernetes 클러스터, NFS 기반 워크로드, 그리고 NetApp 스토리지를 사용하는 iSCSI 및 FC용 VMware vSphere 가상 머신 파일 시스템(VMFS) 데이터스토어를 다음 중 하나로 보호할 수 있습니다:

* Amazon Elastic VMware Service(EVS)와 Amazon FSx for NetApp ONTAP. 자세한 내용은 link:../reference/evs-deploy-guide-introduction.html["Amazon Elastic VMware Service 및 Amazon FSx for NetApp ONTAP를 사용한 NetApp Disaster Recovery 소개"]을 참조하십시오.
* Amazon FSx for NetApp ONTAP 사용한 AWS의 VMware Cloud(VMC)
* Azure VMware Solution with NetApp Cloud Volumes ONTAP(iSCSI)(비공개 미리 보기)
* Google Cloud NetApp Volumes 사용한 Google Cloud VMware Engine(GCVE)
* ONTAP 스토리지를 사용하는 또 다른 온프레미스 NFS 및/또는 VMFS 기반(iSCSI/FC) VMware 환경
...
image:../use/evs-console-architecture.png["NetApp Disaster Recovery의 구성 요소를 보여주는 다이어그램"]
...
== NetApp Disaster Recovery 사용의 이점
...
* 장애 조치 테스트를 수행하십시오. 이 테스트를 수행하면 서비스에서 임시 가상 머신이 생성됩니다. Disaster Recovery는 선택한 스냅샷에서 새 FlexClone 볼륨을 생성하고, FlexClone 볼륨을 기반으로 하는 임시 데이터 저장소가 ESXi 호스트에 매핑됩니다. 이 프로세스는 온프레미스 ONTAP 스토리지 또는 AWS의 NetApp ONTAP 스토리지용 FSx에서 추가적인 물리적 용량을 사용하지 않습니다. 원본 소스 볼륨은 수정되지 않으며, 재해 복구 중에도 복제 작업이 계속될 수 있습니다.
...
image:../use/evs-use-cases.png["NetApp Disaster Recovery의 사용 사례를 보여주는 다이어그램"]


NOTE: vSphere 서버는 vSphere Server에서 NetApp Disaster Recovery 외부에서 구성해야 합니다.
...
* 30일 무료 체험판에 등록하세요.
* Amazon Web Services(AWS) Marketplace, Azure Marketplace 또는 Google Cloud Marketplace에서 종량제(PAYGO) 구독을 구매하십시오. 이 라이선스를 사용하면 장기 계약 없이 고정된 보호 용량 라이선스를 구매할 수 있습니다.
...
재해 복구 서비스는 보호되는 ONTAP 볼륨에 저장된 데이터 용량을 기준으로 라이선스가 부여됩니다. 서비스는 보호되는 가상 머신(VM)을 vCenter 데이터스토어에 매핑하여 라이선스 산정에 고려해야 할 볼륨을 결정합니다. 각 데이터스토어는 ONTAP 볼륨 또는 LUN에 저장됩니다. 해당 볼륨 또는 LUN에 대해 ONTAP에서 보고하는 사용 용량이 라이선스 결정에 사용됩니다.
...
NOTE: NetApp Disaster Recovery 요금은 복제 계획이 있는 VM 또는 Kubernetes 리소스가 하나 이상 있는 경우 소스 사이트의 데이터 저장소 사용 용량에 따라 계산됩니다. 장애 조치된 데이터 저장소의 용량은 허용 용량에 포함되지 않습니다. BYOL의 경우, 데이터가 허용된 용량을 초과하면 추가 용량 라이선스를 구매하거나 NetApp Console에서 라이선스를 업그레이드할 때까지 서비스 내 작업이 제한됩니다.
...
NetApp Disaster Recovery를 30일 무료 체험판으로 사용해 보실 수 있습니다.

30일 평가 기간이 종료된 후에도 계속 사용하려면 클라우드 공급업체로부터 종량제(PAYGO) 구독을 구매하거나 NetApp에서 BYOL 라이센스를 구매해야 합니다.
...
image:dr-architecture-diagram-70-2.png["NetApp Disaster Recovery 서비스 인프라 아키텍처를 보여주는 다이어그램"]
...
Disaster Recovery에는 다음과 같은 구성 요소가 사용됩니다.
...
* Amazon FSx for NetApp ONTAP 사용한 AWS의 VMware Cloud(VMC)
* ONTAP 스토리지를 사용하는 또 다른 온프레미스 NFS 기반 VMware 환경 또는 온프레미스 FC/iSCSI VMFS 환경
