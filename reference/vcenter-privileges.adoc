...
[role="lead"]
vCenter NetApp Disaster Recovery용 vCenter 계정은 데이터스토어를 등록 및 등록 해제하고, 가상 머신(VM)을 시작 및 중지하고, VM을 재구성할 수 있는 최소한의 권한을 보유해야 합니다.

다음 표는 Disaster Recovery가 vCenter 클러스터와 함께 작동하는 데 필요한 권한을 나열합니다.
...
.2+| 호스트 | 호스트.구성.AdvancedConfig | 고급 설정 | 고급 호스트 구성 옵션을 설정할 수 있습니다. 


| 호스트.구성.스토리지 | 스토리지 파티션 구성 | 새로운 스토리지 디바이스를 검색하고 iSCSI를 관리할 수 있습니다. 


| 리소스 | 리소스.VM을 풀에 할당 | 리소스 풀 | 리소스 풀에 VM을 할당할 수 있습니다 
...
.12+| 가상 머신 구성 | VirtualMachine.구성.AdvancedConfig | 고급 구성 | VM의 구성 파일에서 고급 매개 변수를 추가하거나 수정할 수 있습니다. 


| VirtualMachine.구성.AddExistingDisk | 기존 디스크 추가 | 기존 가상 디스크를 VM에 추가할 수 있습니다. 


| VirtualMachine.구성.AddNewDisk | 새 디스크 추가 | 가상 머신에 추가할 새 가상 디스크를 생성할 수 있습니다. 


| VirtualMachine.구성.RemoveDisk | 디스크 제거 | 가상 디스크 장치를 제거할 수 있습니다. 
