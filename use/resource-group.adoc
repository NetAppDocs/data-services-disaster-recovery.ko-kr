...
[role="lead"]
vCenter 사이트를 추가한 후에는 리소스 그룹을 생성하여 VM 또는 데이터 저장소 그룹을 단일 단위로 묶을 수 있습니다. 리소스 그룹을 사용하면 종속 VM 세트를 요구 사항을 충족하는 논리적 그룹으로 구성할 수 있습니다. 예를 들어 하나의 애플리케이션과 연결된 VM을 그룹화하거나 유사한 계층이 있는 애플리케이션을 그룹화할 수 있습니다. 또 다른 예로, 그룹에는 복구 시 실행할 수 있는 지연된 부팅 순서가 포함될 수 있습니다.
...
.리소스 그룹에 관하여
리소스 그룹을 사용하면 VM 또는 데이터 저장소를 단일 단위로 결합할 수 있습니다.
