...
[role="lead"]
리소스 사이트를 추가한 후에는 VM, 데이터스토어 또는 Kubernetes 클러스터를 하나의 단위로 묶는 리소스 그룹을 생성할 수 있습니다. 리소스 그룹을 사용하면 종속 리소스 집합을 요구 사항에 맞는 논리적 그룹으로 구성할 수 있습니다. 예를 들어, 특정 애플리케이션과 관련된 VM을 그룹화하거나 유사한 계층을 가진 애플리케이션을 그룹화할 수 있습니다. 또 다른 예로, 그룹에는 복구 시 실행될 지연 부팅 순서를 포함할 수 있습니다.
...
* 리소스 그룹 옵션에서
* 재해 복구 또는 _복제 계획_을 생성하는 동안 소스 vCenter 클러스터에서 호스팅되는 VM이 많은 경우 복제 계획을 생성하는 동안 리소스 그룹을 생성하는 것이 더 쉬울 수 있습니다. 복제 계획을 생성하는 동안 리소스 그룹을 생성하는 방법에 대한 지침은 link:drplan-create.html["복제 계획 만들기"]을 참조하십시오.
...
. 드롭다운 메뉴에서 *vCenter* 리소스 그룹으로 필터링합니다.
+
...
.단계
. Disaster Recovery에서 사이드바의 **리소스 그룹**을 선택합니다.
. 드롭다운 메뉴에서 *Kubernetes* 사이트를 기준으로 필터링하세요.
+
image::site-type-toggle.png[사이트 유형 전환 스크린샷]
...
+
image:kubernetes-resource-group.png["Kubernetes 리소스 그룹 추가"]
